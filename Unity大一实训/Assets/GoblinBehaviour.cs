...
public class GoblinBehaviour : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GobAnimator = GetComponent<Animator>();
        invokeTime = currentTime;
        Debug.Log("cout invokeTime");
    ...
    void Update()
    {
        ...
        {//自动寻路的脚本
            ...
            if (force.x >= 0.01f)
            {
                enemyGFX.localScale = new Vector3(6.1f, 6.1f, 6.1f);
            }
        ...
        //    Debug.Log("下面是canMove值");
        //    Debug.Log("canMove");
        //    Debug.Log("上面是canMove值");
        //}
        //else
        //{
    ...
    private void OnTriggerExit2D(Collider2D collision)//有两个功能：滑铲离开isTrigger为假以及离开碰撞后不播放攻击动画
    {
    ...
    private void OnTriggerStay2D(Collider2D collision)//这里也可以设置攻击条件
    {
        ...
        if (collision.gameObject.tag == "Player" && !collision.gameObject.GetComponent<Player>().InvincibleEnemy)
        {
            ...
            if (invokeTime >= currentTime)
            {
                ...
                player.healthValue -= damage;//能不能设置为damage?
    ...
    void Animation()
    {
        ...
        {
            Debug.Log(1);
            GobAnimator.SetBool("isAttack", true);
            GobAnimator.SetBool("canMove", false);
        }
        else
        {
            GobAnimator.SetBool("isAttack", false);
            GobAnimator.SetBool("canMove", true);
        }

        //受击动画
        if (isHitted)
        {
            GobAnimator.SetBool("isHitted", true);
            GobAnimator.SetBool("isAttack", false);
            GobAnimator.SetBool("canMove", false);
        }
        else
        ...
        {
            GobAnimator.SetBool("isHitted", true);
            GobAnimator.SetBool("isDead", true);
            GobAnimator.SetBool("isAttack", false);
            GobAnimator.SetBool("canMove", false);
        }
        else
...
/*有个问题，动画效果是可以放出来的，但是不明显，可以考虑再改进*/